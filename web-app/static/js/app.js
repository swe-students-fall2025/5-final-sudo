...
function expiryStatus(expiryDate, leadTimeDays) {
  ...
  if (isNaN(days)) {
    return { status: "Unknown", color: "bg-gray-50 border-gray-200 text-gray-600", icon: "❔", days };
  }

  if (days < 0)
    return { status: "Expired", color: "bg-red-50 border-red-200 text-red-700", icon: "🔴", days };

  if (days <= leadTimeDays)
    return { status: "Action Needed", color: "bg-orange-50 border-orange-200 text-orange-700", icon: "⚠️", days };

  if (days <= leadTimeDays * 2)
    return { status: "Upcoming", color: "bg-yellow-50 border-yellow-200 text-yellow-700", icon: "🟡", days };

  return { status: "Active", color: "bg-green-50 border-green-200 text-green-700", icon: "✅", days };
}
...
function renderDocumentList() {
  ...
  // stats
  const criticalCount = allDocuments.filter(
    (d) => importanceLabel(d.importance) === "Critical"
  ).length;
